...
function clsTurtle.craft(self, item, quantity)	
	...
	elseif item:find("planks") ~= nil then -- craft planks
		sourceSlot = clsTurtle.getItemSlot(self, "log", -1) --get the slot number containing logs
	end
	if sourceSlot == 0 then
		doContinue = false
	end
	if doContinue and chestSlot > 0 then
...
function clsTurtle.dropItem(self, item, direction, keepAmount)
	...
	if keepAmount == nil or keepAmount <= 0 then -- drop everything
		...
		while itemSlot > 0 do
			clsTurtle.drop(self, direction, itemSlot)
			itemSlot = clsTurtle.getItemSlot(self, item, -1)
		end
		...
		while stockData.total > keepAmount do
			if stockData.total - stockData.leastCount > keepAmount then
				clsTurtle.drop(self, direction, stockData.leastSlot)
			else
				clsTurtle.drop(self, direction, stockData.leastSlot, stockData.total - keepAmount)
			end
			stockData = clsTurtle.getStock(self, item)
		end
	end
...
function clsTurtle.getMostItem(self)
...
function clsTurtle.getSolidBlockCount(self)
	local retValue = 0
	local solids = {'stone', 'dirt', 'granite', 'andesite', 'diorite', 'deepslate', 'glass', 'tuff'}
